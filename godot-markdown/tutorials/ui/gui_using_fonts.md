...
[Open Sans](https://fonts.google.com/specimen/Open+Sans) SemiBold
...
[BiDI and Font Features demo project](https://github.com/godotengine/godot-demo-projects/tree/master/gui/bidi_and_font_features).
...
by the [BMFont](https://www.angelcode.com/products/bmfont/) program. Many
BMFont-compatible programs also exist, like [BMGlyph](https://www.bmglyph.com/) or web-based [fontcutter](https://github.com/fabienbk/fontcutter).
...
![Changing import type to Font Data (Image Font)](tutorials/ui/img/using_fonts_bitmap_font_from_image_import_options.webp)
...
[ASCII](https://en.wikipedia.org/wiki/ASCII) characters and follows standard ASCII ordering:

![Bitmap font example](tutorials/ui/img/using_fonts_monospace_bitmap_font_example.webp)

The following import options can be used to import the above font image
successfully:

![Import options to use for the above example font](tutorials/ui/img/using_fonts_bitmap_font_from_image_example_configuration.webp)
...
![Diagram showing the advance and offset values being used in character ranges.](tutorials/ui/img/using_fonts_bitmap_font_advance_offsets_diagram.webp)
...
![image](tutorials/ui/img/using_fonts_bitmap_kerning_pairs_example.webp)
...
![image](tutorials/ui/img/using_fonts_load_font.webp)
...
by setting [CanvasItem.texture_filter](../../classes/class_canvasitem.md#class-canvasitem-property-texture-filter).
...
to [Label3D](../../classes/class_label3d.md#class-label3d).
...
![Font outline example](tutorials/ui/img/using_fonts_outline_example.webp)
...
[Label](../../classes/class_label.md#class-label) and [RichTextLabel](../../classes/class_richtextlabel.md#class-richtextlabel). Additionally, font shadows
...
![Configuring font shadow in a Label node](tutorials/ui/img/using_fonts_shadow.webp)

The result should look like this:

![Font shadow example](tutorials/ui/img/using_fonts_shadow_example.webp)

## Advanced font features

<a id="doc-using-fonts-antialiasing"></a>
...
![Font antialiasing comparison](tutorials/ui/img/using_fonts_antialiasing_comparison.webp)

#### NOTE
Antialiasing cannot be changed on [MSDF-rendered fonts](#doc-using-fonts-msdf)
– these are always rendered with grayscale antialiasing.

<a id="doc-using-fonts-hinting"></a>
...
![Font hinting comparison](tutorials/ui/img/using_fonts_hinting_comparison.webp)
...
Import dock. This will use [FreeType](https://freetype.org/)'s autohinter
to automatically add hinting instructions to the imported font.

<a id="doc-using-fonts-subpixel-positioning"></a>

### Subpixel positioning

Subpixel positioning can be adjusted. This is a [FreeType](https://freetype.org/)
...
<a id="doc-using-fonts-mipmaps"></a>
...
can be especially noticeable with [3D text](../3d/3d_text.md#doc-3d-text) that doesn't have **Fixed
Size** enabled. This can also occur when displaying text with a traditional rasterized
(non-[MSDF](#doc-using-fonts-msdf)) font in a Control node that has its scale
lower than `(1, 1)`.
...
- Rendering new glyphs for the first time *at small font sizes* may be more
  ...
  [Font prerendering](#doc-using-fonts-font-prerendering) can be used to alleviate this.
...
- Fonts with self-intersecting outlines will not render correctly in MSDF mode.
  ...
  [Google Fonts](https://fonts.google.com), try downloading the font from the
  font author's official website instead.

![Comparison of font rasterization methods](tutorials/ui/img/using_fonts_rasterized_vs_msdf_comparison.webp)
...
![Enabling MSDF in the font's import options](tutorials/ui/img/using_fonts_msdf_import_options.webp)

<a id="doc-using-fonts-emoji"></a>
...
[fallbacks](#doc-using-fonts-font-fallbacks)) needs to include them.
Otherwise, emoji won't be displayed and placeholder "tofu" characters will
appear instead:

![Default appearance when trying to use emoji in a label](tutorials/ui/img/using_fonts_emoji_placeholder_characters.webp)

After adding a font to display emoji such as
[Noto Color Emoji](https://fonts.google.com/noto/specimen/Noto+Color+Emoji),
you get the expected result:

![Correct appearance after adding an emoji font to the label](tutorials/ui/img/using_fonts_emoji_correct_characters.webp)

To use a regular font alongside emoji, it's recommended to specify a
[fallback font](#doc-using-fonts-font-fallbacks) that points to the
...
Tools like [Fontello](https://fontello.com/) can be used to generate font
...
with [3D text](../3d/3d_text.md#doc-3d-text) and TextMesh.
...
the `img` tag in [RichTextLabel](bbcode_in_richtextlabel.md#doc-bbcode-in-richtextlabel). Unlike
...
specify the icon font as a [fallback](#doc-using-fonts-font-fallbacks).
...
#### NOTE
Several modern icon fonts such as [Font Awesome 6](https://fontawesome.com/download)
...
<a id="doc-using-fonts-font-fallbacks"></a>
...
![image](tutorials/ui/img/using_fonts_advanced_import_settings.webp)
...
![image](tutorials/ui/img/using_fonts_font_fallbacks_add.webp)

Click the dropdown arrow on the new element, then choose a font file using the
**Quick Load** or **Load** options:

![image](tutorials/ui/img/using_fonts_font_fallbacks_load.webp)
...
[OpenType font features](#doc-using-fonts-opentype-font-features), but this is not covered here
for brevity reasons.

<a id="doc-using-fonts-variable-fonts"></a>

### Variable fonts

Godot has full support for [variable fonts](https://variablefonts.io/), which
...
To use a variable font, create a [FontVariation](../../classes/class_fontvariation.md#class-fontvariation) resource in the
location where you intend to use the font, then load a font file within the
FontVariation resource:

![image](tutorials/ui/img/using_fonts_font_variation_create.webp)![image](tutorials/ui/img/using_fonts_font_variation_load.webp)
...
![image](tutorials/ui/img/using_fonts_font_variation_variable_font.webp)
...
![image](tutorials/ui/img/using_fonts_font_variation_variable_font_example.webp)

You can save the FontVariation to a `.tres` resource file to reuse it in other
places:

![image](tutorials/ui/img/using_fonts_font_variation_save_to_file.webp)
...
increases distribution size. A single [variable font](#doc-using-fonts-variable-fonts)
...
![Faux bold/italic (top), real bold/italic (bottom). Normal font used: Open Sans SemiBold](tutorials/ui/img/using_fonts_faux_bold_italic_vs_real_bold_italic.webp)

Faux bold and italic is automatically used in [RichTextLabel](../../classes/class_richtextlabel.md#class-richtextlabel)'s bold
...
- **Top:** Additional spacing above glyphs. This is used for multiline text,
  but also to calculate the minimum size of controls such as [Label](../../classes/class_label.md#class-label)
  and [Button](../../classes/class_button.md#class-button).
...
<a id="doc-using-fonts-opentype-font-features"></a>
...
![image](tutorials/ui/img/using_fonts_advanced_import_settings_opentype_features.webp)

**In a specific font usage (FontVariation)**

To use a font feature, create a FontVariation resource like you would do for a
[variable font](#doc-using-fonts-variable-fonts), then load a font file
...
![image](tutorials/ui/img/using_fonts_font_variation_opentype_features.webp)
...
![image](tutorials/ui/img/using_fonts_font_variation_slashed_zero.webp)
...
![image](tutorials/ui/img/using_fonts_font_variation_disable_ligatures.webp)

<a id="doc-using-fonts-system-fonts"></a>
...
[Using emoji](#doc-using-fonts-emoji) section.

Create a [SystemFont](../../classes/class_systemfont.md#class-systemfont) resource in the location where you desire to
use the system font:

![image](tutorials/ui/img/using_fonts_system_font_create.webp)![image](tutorials/ui/img/using_fonts_system_font_specify.webp)
...
See [Runtime loading and saving](../io/runtime_file_loading_and_saving.md#doc-runtime-file-loading-and-saving-fonts)
for details.

<a id="doc-using-fonts-font-prerendering"></a>
...
![Adding a new prerendering configuration in the Advanced Import Settings dialog](tutorials/ui/img/using_fonts_advanced_import_settings_prerender_new_configuration.webp)
...
[internationalization](../i18n/internationalizing_games.md#doc-internationalizing-games). Otherwise, stick to
...
![Enabling prerendering in the Advanced Import Settings dialog with the Glyphs from the Translations tab](tutorials/ui/img/using_fonts_advanced_import_settings_prerender_translation.webp)
...
![Enabling prerendering in the Advanced Import Settings dialog, Glyphs from the Text tab](tutorials/ui/img/using_fonts_advanced_import_settings_prerender_text.webp)
...
![Enabling prerendering in the Advanced Import Settings dialog, Glyphs from the Character Map tab](tutorials/ui/img/using_fonts_advanced_import_settings_prerender_character_map.webp)
