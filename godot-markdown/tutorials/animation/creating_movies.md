...
- Recording cutscenes that will be [displayed as pre-recorded videos](playing_videos.md#doc-playing-videos)
...
- Render at a higher framerate than the video's target framerate, then
  [post-process to generate high-quality motion blur](#doc-creating-movies-motion-blur).
...
Players should use something like [OBS Studio](https://obsproject.com/) or
...
![Movie Maker mode is disabled, click the "movie reel" icon to enable](tutorials/animation/img/creating_movies_enable_movie_maker_mode.webp)
...
![Movie Maker mode is enabled, click the "movie reel" icon again to disable](tutorials/animation/img/creating_movies_disable_movie_maker_mode.webp)
...
![Movie Maker project settings (with Advanced toggle enabled)](tutorials/animation/img/creating_movies_project_settings.webp)
...
![Inspector view after creating a ``movie_file`` metadata of type String](tutorials/animation/img/creating_movies_set_per_scene_metadata.webp)
...
Movie Maker can also be enabled from the [command line](../editor/command_line_tutorial.md#doc-command-line-tutorial):
...
`disabled` or `canvas_items` [stretch mode](../rendering/multiple_resolutions.md#doc-multiple-resolutions):
...
[Rendering at a higher resolution than the screen resolution](#doc-creating-movies-recording-at-higher-resolution) if you need to record
...
but you can use the [OS.execute()](../../classes/class_os.md#class-os-method-execute) method to
run a second instance of the exported project that will record a video file.

## Choosing an output format

Output formats are provided by the [MovieWriter](../../classes/class_moviewriter.md#class-moviewriter) class.
Godot has 3 built-in [MovieWriters](../../classes/class_moviewriter.md#class-moviewriter), and more can be
...
The resulting file can be viewed in Godot with [VideoStreamPlayer](../../classes/class_videostreamplayer.md#class-videostreamplayer) and most video players but not web browsers. OGV
...
On the other hand, [OGV playback](playing_videos.md#doc-playing-videos)
...
to be
[encoded to a video file with an external tool after recording](#doc-creating-movies-converting-avi).
...
#### NOTE
When using the `disabled` or `2d` [stretch modes](../rendering/multiple_resolutions.md#doc-multiple-resolutions),
...
See also [Rendering at a higher resolution than the screen resolution](#doc-creating-movies-recording-at-higher-resolution).
...
[Using a video converter program](#doc-creating-movies-converting-avi)
...
The `movie` [feature tag](../export/feature_tags.md#doc-feature-tags) can be used to override
...
<a id="doc-creating-movies-recording-at-higher-resolution"></a>
...
By default, Godot uses the `disabled` [stretch modes](../rendering/multiple_resolutions.md#doc-multiple-resolutions)
...
those settings with the `movie` [feature tag](../export/feature_tags.md#doc-feature-tags).
...
<a id="doc-creating-movies-converting-avi"></a>

### Converting OGV/AVI video to MP4

While some platforms such as YouTube support uploading the AVI file directly, many
others will require a conversion step beforehand. [HandBrake](https://handbrake.fr/)
...
<a id="doc-creating-movies-converting-image-sequence"></a>
...
<a id="doc-creating-movies-motion-blur"></a>
...
<a id="doc-creating-movies-reducing-framerate"></a>
...
See [Reducing framerate](#doc-creating-movies-reducing-framerate) if you want to get this benefit
