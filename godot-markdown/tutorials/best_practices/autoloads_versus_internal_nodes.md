<a id="doc-autoloads-versus-internal-nodes"></a>
...
[Singletons (Autoload)](../scripting/singletons_autoload.md#doc-singletons-autoload). These autoloaded nodes are not freed when you
change the scene from code with [SceneTree.change_scene_to_file](../../classes/class_scenetree.md#class-scenetree-method-change-scene-to-file).
...
that play a sound effect. There's a node for that: the [AudioStreamPlayer](../../classes/class_audiostreamplayer.md#class-audiostreamplayer). But if we call the `AudioStreamPlayer` while it is
...
that feature for an individual scene using the [class_name](../scripting/gdscript/gdscript_basics.md#doc-gdscript-basics-class-name) keyword in GDScript.

When it comes to data, you can either:

1. Create a new type of [Resource](../../classes/class_resource.md#class-resource) to share the data.
