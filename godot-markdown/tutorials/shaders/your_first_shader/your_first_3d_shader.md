<a id="doc-your-first-spatial-shader"></a>
...
[StandardMaterial3D](../../../classes/class_standardmaterial3d.md#class-standardmaterial3d) isn't quite meeting your
needs. Either way, you have decided to write your own and now you need to figure
out where to start.

This tutorial will explain how to write a Spatial shader and will cover more
topics than the [CanvasItem](your_first_2d_shader.md#doc-your-first-canvasitem-shader) tutorial.
...
vertex function. In the [second part](your_second_3d_shader.md#doc-your-second-spatial-shader) we
...
introduction from [The Book of Shaders](https://thebookofshaders.com) before completing this tutorial.

## Where to assign my material

In 3D, objects are drawn using [Meshes](../../../classes/class_mesh.md#class-mesh). Meshes are a resource
...
program (e.g. Blender). But Godot also has a few [PrimitiveMeshes](../../../classes/class_primitivemesh.md#class-primitivemesh) that allow you to add basic geometry to a scene without
importing Meshes.

There are multiple node types that you can use to draw a mesh. The main one is
[MeshInstance3D](../../../classes/class_meshinstance3d.md#class-meshinstance3d), but you can also use [GPUParticles3D](../../../classes/class_gpuparticles3d.md#class-gpuparticles3d), [MultiMeshes](../../../classes/class_multimesh.md#class-multimesh) (with a
[MultiMeshInstance3D](../../../classes/class_multimeshinstance3d.md#class-multimeshinstance3d)), or others.
...
Add a new [MeshInstance3D](../../../classes/class_meshinstance3d.md#class-meshinstance3d) node to your scene.

In the inspector tab, set the MeshInstance3D's **Mesh** property to a new
[PlaneMesh](../../../classes/class_planemesh.md#class-planemesh) resource, by clicking on `<empty>` and
...
![image](tutorials/shaders/your_first_shader/img/plane.webp)

Now set **Subdivide Width** and **Subdivide Depth** of the [PlaneMesh](../../../classes/class_planemesh.md#class-planemesh) to `32`.

![image](tutorials/shaders/your_first_shader/img/plane-sub-set.webp)

You can see that there are now many more triangles in the
[MeshInstance3D](../../../classes/class_meshinstance3d.md#class-meshinstance3d). This will give us more vertices to work with
and thus allow us to add more detail.

![image](tutorials/shaders/your_first_shader/img/plane-sub.webp)

[PrimitiveMeshes](../../../classes/class_primitivemesh.md#class-primitivemesh), like PlaneMesh, only have one
...
#### NOTE
Materials that inherit from the [Material](../../../classes/class_material.md#class-material) resource, such as [StandardMaterial3D](../../../classes/class_standardmaterial3d.md#class-standardmaterial3d)
and [ParticleProcessMaterial](../../../classes/class_particleprocessmaterial.md#class-particleprocessmaterial), can be converted to a [ShaderMaterial](../../../classes/class_shadermaterial.md#class-shadermaterial)
...
![image](tutorials/shaders/your_first_shader/img/shader-editor.webp)
...
The `vertex()` function determines where the vertices of your [MeshInstance3D](../../../classes/class_meshinstance3d.md#class-meshinstance3d)
...
![image](tutorials/shaders/your_first_shader/img/cos.webp)
...
to [cos()](../shader_reference/shader_functions.md#shader-func-cos) and [sin()](../shader_reference/shader_functions.md#shader-func-sin); that gives
...
![image](tutorials/shaders/your_first_shader/img/cos4.webp)
...
Godot provides the [NoiseTexture2D](../../../classes/class_noisetexture2d.md#class-noisetexture2d) resource for
...
Set this **Noise** parameter to a new [NoiseTexture2D](../../../classes/class_noisetexture2d.md#class-noisetexture2d).
Then in your NoiseTexture2D, set its **Noise** property to a new
[FastNoiseLite](../../../classes/class_fastnoiselite.md#class-fastnoiselite). The FastNoiseLite class is used by
the NoiseTexture2D to generate a heightmap.

Once you set it up and should look like this.

![image](tutorials/shaders/your_first_shader/img/noise-set.webp)
...
[texture()](../shader_reference/shader_functions.md#shader-func-texture) takes a texture as the first argument and
...
above, we could use `texture().r`. See the [OpenGL documentation](https://www.khronos.org/opengl/wiki/Data_Type_(GLSL)#Vectors) for more
details.

Using this code you can see the texture creates random looking hills.

![image](tutorials/shaders/your_first_shader/img/noise.webp)
...
[Uniform variables](../shader_reference/shading_language.md#doc-shading-language-uniforms) allow you to pass data
...
[Shader](../../../classes/class_shader.md#class-shader) using the keyword `uniform`.
...
[set_shader_parameter()](../../../classes/class_shadermaterial.md#class-shadermaterial-method-set-shader-parameter)
...
![image](tutorials/shaders/your_first_shader/img/noise-low.webp)

Using uniforms, we can even change the value every frame to animate the height
of the terrain. Combined with [Tweens](../../../classes/class_tween.md#class-tween), this can be
...
![image](tutorials/shaders/your_first_shader/img/normal.webp)

Note how the mesh color goes flat. This is because the lighting on it is flat.
Let's add a light!

First, we will add an [OmniLight3D](../../../classes/class_omnilight3d.md#class-omnilight3d) to the scene, and
drag it up so it is above the terrain.

![image](tutorials/shaders/your_first_shader/img/light.webp)
...
the light shader uses the normals from the [Mesh](../../../classes/class_mesh.md#class-mesh) to calculate
light.
...
Set this second uniform texture to another [NoiseTexture2D](../../../classes/class_noisetexture2d.md#class-noisetexture2d) with another
[FastNoiseLite](../../../classes/class_fastnoiselite.md#class-fastnoiselite). But this time, check **As Normal Map**.

![image](tutorials/shaders/your_first_shader/img/normal-set.webp)
...
that using a [varying](../shader_reference/shading_language.md#doc-shading-language-varyings).
...
![image](tutorials/shaders/your_first_shader/img/normalmap.webp)

We can even drag the light around and the lighting will update automatically.

![image](tutorials/shaders/your_first_shader/img/normalmap2.webp)
