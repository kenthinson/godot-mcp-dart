<a id="doc-viewport-as-texture"></a>

# Using a SubViewport as a texture

## Introduction

This tutorial will introduce you to using the [SubViewport](../../classes/class_subviewport.md#class-subviewport) as a
...
![image](tutorials/shaders/img/planet_example.png)
...
a [Camera3D](../../classes/class_camera3d.md#class-camera3d), a [light source](../../classes/class_omnilight3d.md#class-omnilight3d), a
[MeshInstance3D](../../classes/class_meshinstance3d.md#class-meshinstance3d) with a [Primitive Mesh](../../classes/class_primitivemesh.md#class-primitivemesh),
and applying a [StandardMaterial3D](../../classes/class_standardmaterial3d.md#class-standardmaterial3d) to the mesh. The focus will be on using
the [SubViewport](../../classes/class_subviewport.md#class-subviewport) to dynamically create textures that can be applied to the mesh.

In this tutorial, we'll cover the following topics:

- How to use a [SubViewport](../../classes/class_subviewport.md#class-subviewport) as a render texture
- Mapping a texture to a sphere with equirectangular mapping
- Fragment shader techniques for procedural planets
- Setting a Roughness map from a [Viewport Texture](../../classes/class_viewporttexture.md#class-viewporttexture)

## Setting up the scene

Create a new scene and add the following nodes exactly as shown below.

![image](tutorials/shaders/img/viewport_texture_node_tree.webp)

Go into the the MeshInstance3D and make the mesh a SphereMesh

## Setting up the SubViewport

Click on the [SubViewport](../../classes/class_subviewport.md#class-subviewport) node and set its size to `(1024, 512)`. The
[SubViewport](../../classes/class_subviewport.md#class-subviewport) can actually be any size so long as the width is double the
...
Next disable 3D. We will be using a [ColorRect](../../classes/class_colorrect.md#class-colorrect) to render the surface, so
we don't need 3D either.

![image](tutorials/shaders/img/planet_new_viewport.webp)

Select the [ColorRect](../../classes/class_colorrect.md#class-colorrect) and in the inspector set the anchors preset to `Full Rect`.
This will ensure that the [ColorRect](../../classes/class_colorrect.md#class-colorrect) takes up the entire [SubViewport](../../classes/class_subviewport.md#class-subviewport).

![image](tutorials/shaders/img/planet_new_colorrect.webp)

Next, we add a [Shader Material](../../classes/class_shadermaterial.md#class-shadermaterial) to the [ColorRect](../../classes/class_colorrect.md#class-colorrect) (ColorRect > CanvasItem > Material > Material > `New ShaderMaterial`).
...
![image](tutorials/shaders/img/planet_gradient.png)

Now we have the basics of a [SubViewport](../../classes/class_subviewport.md#class-subviewport) that we render to and we have a unique image that we can
apply to the sphere.

## Applying the texture

Now go into the [MeshInstance3D](../../classes/class_meshinstance3d.md#class-meshinstance3d) and add a [StandardMaterial3D](../../classes/class_standardmaterial3d.md#class-standardmaterial3d)
to it. No need for a special [Shader Material](../../classes/class_shadermaterial.md#class-shadermaterial) (although that would be a good idea
...
![image](tutorials/shaders/img/planet_new_viewport_texture.webp)
...
![image](tutorials/shaders/img/planet_pick_viewport_texture.webp)

Your sphere should now be colored in with the colors we rendered to the Viewport.

![image](tutorials/shaders/img/planet_seam.webp)
...
So now, when we render to our [SubViewport](../../classes/class_subviewport.md#class-subviewport), it appears magically on the sphere. But there is an ugly
...
![image](tutorials/shaders/img/planet_sincos.webp)
...
[StandardMaterial3D](../../classes/class_standardmaterial3d.md#class-standardmaterial3d). It uses a projection technique called equirectangular
...
![image](tutorials/shaders/img/planet_normals.webp)
...
![image](tutorials/shaders/img/planet_noise.webp)
...
![image](tutorials/shaders/img/planet_noise_color.webp)
...
![image](tutorials/shaders/img/planet_noise_smooth.webp)
...
![image](tutorials/shaders/img/planet_noise_fbm.webp)
...
[Viewport Texture](../../classes/class_viewporttexture.md#class-viewporttexture) pointing to our planet texture [SubViewport](../../classes/class_subviewport.md#class-subviewport).
...
![image](tutorials/shaders/img/planet_ocean.webp)
...
of the [SubViewport](../../classes/class_subviewport.md#class-subviewport) is opaque, the `alpha` channel of the
[Viewport Texture](../../classes/class_viewporttexture.md#class-viewporttexture) is `1`, resulting in the planet texture being
drawn with slightly fainter colors and a `Roughness` value of `1` everywhere. To correct this, we
go into the [SubViewport](../../classes/class_subviewport.md#class-subviewport) and enable the "Transparent Bg" property. Since we are now
...
Now the planet should look like it is reflecting light on the ocean but not the land. move around the [OmniLight3D](../../classes/class_omnilight3d.md#class-omnilight3d)
in the scene so you can see the effect of the reflections on the ocean.

![image](tutorials/shaders/img/planet_ocean_reflect.webp)

And there you have it. A procedural planet generated using a [SubViewport](../../classes/class_subviewport.md#class-subviewport).
