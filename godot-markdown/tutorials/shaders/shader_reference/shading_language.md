...
Guide](../converting_glsl_to_godot_shaders.md#doc-converting-glsl-to-godot-shaders) is a resource that will help you
transition from regular GLSL to Godot's shading language.

<a id="doc-shading-language-data-types"></a>
...
| **samplerCubeArray**   | Sampler type for binding Cubemap arrays, which are read as float.<br/>Only supported in Forward+ and Mobile, not Compatibility. |
| **samplerExternalOES** | External sampler type.<br/>Only supported in Compatibility/Android platform.                                                    |

These types can also be put inside [arrays](#doc-shading-language-arrays)
or [structs](#doc-shading-language-structs), which are also usable as function parameters
...
per the [Shaders style guide](../shaders_style_guide.md#doc-shaders-style-guide). These asterisks are automatically
...
<a id="doc-shading-language-arrays"></a>
...
<a id="doc-shading-language-structs"></a>
...
name, but different arguments. Note that [implicit casting]()
...
<a id="doc-shading-language-varyings"></a>
...
<a id="doc-shading-language-uniforms"></a>
...
[data type](#doc-shading-language-data-types) except for `void` can be a uniform.
...
can set them [from code](#doc-shading-language-setting-uniforms-from-code).
...
| **sampler2D**  | hint_roughness[_r, \_g, \_b, \_a, \_normal, \_gray] | Used for roughness limiter on import (attempts reducing specular aliasing).<br/>`_normal` is a normal map that guides the roughness limiter,<br/>with roughness increasing in areas that have high-frequency detail. |
...
and in `canvas_item` shaders when [HDR 2D](../../../classes/class_projectsettings.md#class-projectsettings-property-rendering-viewport-hdr-2d)
...
<a id="doc-shading-language-global-uniforms"></a>
...
![Adding a global uniform in the Shader Globals tab of the Project Settings](tutorials/shaders/shader_reference/img/shading_language_adding_global_uniforms.webp)
...
[RenderingServer.global_shader_parameter_set](../../../classes/class_renderingserver.md#class-renderingserver-method-global-shader-parameter-set)
method in a script:
...
setting them, consider creating an [autoload](../../scripting/singletons_autoload.md#doc-singletons-autoload)
where you store the values you need to query at the same time you're setting
them as global uniforms.

<a id="doc-shading-language-per-instance-uniforms"></a>
...
![Setting a per-instance uniform's value in the GeometryInstance3D section of the inspector](tutorials/shaders/shader_reference/img/shading_language_per_instance_uniforms_inspector.webp)

Per-instance uniform values can also be set at runtime using
[set_instance_shader_parameter](../../../classes/class_geometryinstance3d.md#class-geometryinstance3d-method-set-instance-shader-parameter)
method on a node that inherits from [GeometryInstance3D](../../../classes/class_geometryinstance3d.md#class-geometryinstance3d):
...
<a id="doc-shading-language-setting-uniforms-from-code"></a>
...
|------------------------|----------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **bool**               | **bool**                                                             |                                                                                                                                                                                                                                                               |
| **bvec2**              | **int**                                                              | Bitwise packed int where bit 0 (LSB) corresponds to x.<br/><br/>For example, a bvec2 of (bx, by) could be created in<br/>the following way:<br/><br/>```gdscript<br/>bvec2_input: int = (int(bx)) | (int(by) << 1)<br/>```                                    |
...
| **vec4**               | **Vector4**, **Color**,<br/>**Rect2**, **Plane**,<br/>**Quaternion** | When Color is used, it will be interpreted as (r, g, b, a).<br/><br/>When Rect2 is used, it will be interpreted as<br/>(position.x, position.y, size.x, size.y).<br/><br/>When Plane is used it will be interpreted as<br/>(normal.x, normal.y, normal.z, d). |
| **mat2**               | **Transform2D**                                                      |                                                                                                                                                                                                                                                               |
| **mat3**               | **Basis**                                                            |                                                                                                                                                                                                                                                               |
| **mat4**               | **Projection**,<br/>**Transform3D**                                  | When a Transform3D is used, the w Vector is set to the<br/>identity.                                                                                                                                                                                          |
| **sampler2D**          | **Texture2D**                                                        |                                                                                                                                                                                                                                                               |
...
| **samplerCube**        | **Cubemap**                                                          | See [Changing import type](../../assets_pipeline/importing_images.md#doc-importing-images-changing-import-type) for<br/>instructions on importing cubemaps for use in Godot.                                                                                  |
...
- [Spatial shaders](spatial_shader.md#doc-spatial-shader)
- [Canvas item shaders](canvas_item_shader.md#doc-canvas-item-shader)
- [Particle shaders](particle_shader.md#doc-particle-shader)
- [Sky shaders](sky_shader.md#doc-sky-shader)
- [Fog shaders](fog_shader.md#doc-fog-shader)

## Built-in functions

A large number of built-in functions are supported, conforming to GLSL ES 3.0.
See the [Built-in functions](shader_functions.md#doc-shader-functions) page for details.
