<a id="doc-multiple-resolutions"></a>
...
to play the game (given their hardware). For mobile, Google has useful [stats](https://developer.android.com/about/dashboards) online, and for desktop,
...
[Multiple Resolutions and Aspect Ratios demo project](https://github.com/godotengine/godot-demo-projects/tree/master/gui/multiple_resolutions).
...
![image](tutorials/rendering/img/screenres.webp)
...
This viewport is a [Window](../../classes/class_window.md#class-window) instance. Recall from the
[introduction](viewports.md#doc-viewports) that *all* Window objects are viewports.

To configure the stretch base size at runtime from a script, use the
`get_tree().root.content_scale_size` property (see
[Window.content_scale_size](../../classes/class_window.md#class-window-property-content-scale-size)).
...
[Stretch Scale](#doc-multiple-resolutions-stretch-scale) is recommended as it's easier to
adjust.
...
little easier. The [Viewport](../../classes/class_viewport.md#class-viewport)
...
![image](tutorials/rendering/img/stretchsettings.webp)
...
and a diagonal [Line2D](../../classes/class_line2d.md#class-line2d) is added on top of it:

![image](tutorials/rendering/img/stretch_demo_scene.png)
...
- **Stretch Mode = Disabled** (default): No stretching happens. One
  ...
  ![image](tutorials/rendering/img/stretch_disabled_expand.gif)
- **Stretch Mode = Canvas Items**: In this mode, the base size specified in
  ...
  ![image](tutorials/rendering/img/stretch_2d_expand.gif)
- **Stretch Mode = Viewport**: Viewport scaling means that the size of
  the root [Viewport](../../classes/class_viewport.md#class-viewport) is set precisely to the
  ...
  account).
  ![image](tutorials/rendering/img/stretch_viewport_expand.gif)

To configure the stretch mode at runtime from a script, use the
`get_tree().root.content_scale_mode` property (see
[Window.content_scale_mode](../../classes/class_window.md#class-window-property-content-scale-mode)
and the [ContentScaleMode](../../classes/class_window.md#enum-window-contentscalemode) enum).
...
- **Stretch Aspect = Ignore**: Ignore the aspect ratio when stretching
  ...
  designed.
  ![image](tutorials/rendering/img/stretch_viewport_ignore.gif)
- **Stretch Aspect = Keep**: Keep aspect ratio when stretching the
  ...
  ratios.
  ![image](tutorials/rendering/img/stretch_viewport_keep.gif)
- **Stretch Aspect = Keep Width**: Keep aspect ratio when stretching the
  ...
  ([Size and anchors](../ui/size_and_anchors.md#doc-size-and-anchors)).
  ![image](tutorials/rendering/img/stretch_viewport_keep_width.gif)
- **Stretch Aspect = Keep Height**: Keep aspect ratio when stretching
  ...
  ![image](tutorials/rendering/img/stretch_viewport_keep_height.gif)
...
[Window.content_scale_aspect](../../classes/class_window.md#class-window-property-content-scale-aspect)
and the [ContentScaleAspect](../../classes/class_window.md#enum-window-contentscaleaspect) enum).

<a id="doc-multiple-resolutions-stretch-scale"></a>
...
[Window.content_scale_factor](../../classes/class_window.md#class-window-property-content-scale-factor)).
...
<a id="doc-multiple-resolutions-stretch-scale-mode"></a>
...
![Fractional scaling example (incorrect pixel art appearance)](tutorials/rendering/img/multiple_resolutions_pixel_art_fractional_scaling.webp)

This pixel art is also displayed with the `viewport` stretch mode, but the
stretch scale mode is set to `integer` this time:

![Integer scaling example (correct pixel art appearance)](tutorials/rendering/img/multiple_resolutions_pixel_art_integer_scaling.webp)
...
- Alternatively, if you're targeting high-end devices primarily, set the base
  ...
  - Note that this will make non-mipmapped textures grainy on low resolution devices,
    ...
    [Reducing aliasing on downsampling](#doc-multiple-resolutions-reducing-aliasing-on-downsampling).
...
- For 3D games, consider exposing [Resolution scaling](../3d/resolution_scaling.md#doc-resolution-scaling) in the game's options menu
...
<a id="doc-multiple-resolutions-non-game-application"></a>
...
- Add a setting in the application's settings to change the root viewport's
  [stretch scale](#doc-multiple-resolutions-stretch-scale),
...
[autoload](../scripting/singletons_autoload.md#doc-singletons-autoload)'s `_ready()` function according to
the screen size.
...
- For non-game applications, use the `disabled` stretch mode and set the
  ...
  [autoload](../scripting/singletons_autoload.md#doc-singletons-autoload)'s `_ready()` function.
  The display scale factor is set in the operating system's settings and can be queried
  using [screen_get_scale](../../classes/class_displayserver.md#class-displayserver-method-screen-get-scale). This
...
<a id="doc-multiple-resolutions-reducing-aliasing-on-downsampling"></a>
...
To resolve this, you can [enable mipmaps](../assets_pipeline/importing_images.md#doc-importing-images-mipmaps) on
...
done using [anchors](../ui/size_and_anchors.md#doc-size-and-anchors) and/or [containers](../ui/gui_containers.md#doc-gui-containers).
...
[resolution scaling](../3d/resolution_scaling.md#doc-resolution-scaling) functionality. This allows you to
