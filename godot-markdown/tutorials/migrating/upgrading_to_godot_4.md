<a id="doc-upgrading-to-godot-4"></a>
...
Along with the
[new features present in 4.0](https://godotengine.org/article/godot-4-0-sets-sail),
...
- 4.x will enjoy a longer [support period](../../about/release_policy.md#doc-release-policy). Godot 3.x
  will continue to be supported for some time after 4.0 is released, but it will
  eventually stop receiving support.

See [Documentation changelog](../../about/docs_changelog.md#doc-docs-changelog) for a list of pages documenting new features in
Godot 4.0, and [List of features](../../about/list_of_features.md#doc-list-of-features) for a list of all features in Godot.
...
- Since Godot 4 includes more features than Godot 3, Godot 4's binary size for
  ...
  [optimizing a build for size](../../engine_details/development/compiling/optimizing_for_size.md#doc-optimizing-for-size), a 4.0 build with
  a given set of enabled modules will remain larger compared to a 3.x build with
  the same modules. This can be an issue for
  [exporting to the Web](../export/exporting_for_web.md#doc-exporting-for-web), as binary size directly
...
- Godot 4 does not and will not have support for GLES2 rendering.
  ...
  - If you are targeting **very** old hardware such as Intel Sandy Bridge (2nd
    ...
    [Software OpenGL implementations](https://github.com/pal1000/mesa-dist-win)
...
- By default, rendering in 2D is no longer performed in HDR, which means
  ...
  enable the project setting [HDR 2D](../../classes/class_projectsettings.md#class-projectsettings-property-rendering-viewport-hdr-2d)
  to perform 2D rendering in HDR. See also [Using glow in 2D](../3d/environment_and_post_processing.md#doc-environment-and-post-processing-using-glow-in-2d).
...
[issues labeled "regression" but not "bug" on GitHub](https://github.com/godotengine/godot/issues?q=is%3Aissue+is%3Aopen+label%3Aregression+-label%3Abug).

## Preparing before the upgrade (optional)

If you want to be ready to upgrade to Godot 4 in the future, consider using
[Tweener](../../classes/class_tweener.md#class-tweener) and the [Time](../../classes/class_time.md#class-time) singleton in your project. These
...
To use the project upgrade tool from the [command line](../editor/command_line_tutorial.md#doc-command-line-tutorial),
...
- File and Directory classes were replaced by [FileAccess](../../classes/class_fileaccess.md#class-fileaccess) and
  [DirAccess](../../classes/class_diraccess.md#class-diraccess), which have an entirely different API. Several methods
...
- Screen and window-related methods from the [OS](../../classes/class_os.md#class-os) singleton (such as
  `OS.get_screen_size()`) were moved to the [DisplayServer](../../classes/class_displayserver.md#class-displayserver) singleton.
...
- Time and date methods from the [OS](../../classes/class_os.md#class-os) singleton were moved to the
  [Time](../../classes/class_time.md#class-time) singleton.
...
- AnimationPlayer's `add_animation()` is now `add_animation_library()` and now uses an [AnimationLibrary](../../classes/class_animationlibrary.md#class-animationlibrary).
...
Godot 3.x, you will have to change its code to call [RenderingServer](../../classes/class_renderingserver.md#class-renderingserver)
...
- [Built in matrix variables were renamed.](../shaders/shader_reference/spatial_shader.md#doc-spatial-shader)
...
See [Shading language](../shaders/shader_reference/shading_language.md#doc-shading-language) for more information.

This list is not exhaustive. If you made all the changes mentioned here and your
shader still doesn't work, try asking for help in one of the [community channels](https://godotengine.org/community/).
...
- Both [String](../../classes/class_string.md#class-string) and [StringName](../../classes/class_stringname.md#class-stringname) are now exposed to
...
- [GDScript setter and getter syntax](../scripting/gdscript/gdscript_basics.md#doc-gdscript-basics-setters-getters)
...
- [GDScript signal connection syntax](../scripting/gdscript/gdscript_basics.md#doc-gdscript-signals) was changed.
  The conversion tool will use the string-based syntax which is still present in
  Godot 4, but it's recommended to switch to the [Signal](../../classes/class_signal.md#class-signal)-based syntax
...
- Built-in scripts that are [tool scripts](../plugins/running_code_in_the_editor.md#doc-running-code-in-the-editor)
...
- The Tween node was removed in favor of Tweeners, which are also available in
  ...
  [original pull request](https://github.com/godotengine/godot/pull/41794)
  for details.
...
- Instead of `rotation_degrees`, the `rotation` property is exposed to the
  ...
  conversion tool.
- [AABB](../../classes/class_aabb.md#class-aabb)'s `has_no_surface()` was inverted and renamed to `has_surface()`.
- [AABB](../../classes/class_aabb.md#class-aabb) and [Rect2](../../classes/class_rect2.md#class-rect2)'s `has_no_area()` was inverted and
  renamed to `has_area()`.
- [AnimatedTexture](../../classes/class_animatedtexture.md#class-animatedtexture)'s `fps` property was replaced by `speed_scale`,
  which works the same as AnimationPlayer's `playback_speed` property.
- [AnimatedSprite2D](../../classes/class_animatedsprite2d.md#class-animatedsprite2d) and [AnimatedSprite3D](../../classes/class_animatedsprite3d.md#class-animatedsprite3d) now allow
  negative `speed_scale` values. This may break animations if you relied on
  `speed_scale` being internally clamped to `0.0`.
- [AnimatedSprite2D](../../classes/class_animatedsprite2d.md#class-animatedsprite2d) and [AnimatedSprite3D](../../classes/class_animatedsprite3d.md#class-animatedsprite3d)'s `playing`
...
- [Array](../../classes/class_array.md#class-array)'s `slice()` second parameter (`end`) is now *exclusive*,
  instead of being inclusive. For example, this means that
  `[1, 2, 3].slice(0, 1)` now returns `[1]` instead of `[1, 2]`.
- [BaseButton](../../classes/class_basebutton.md#class-basebutton)'s signals are now `button_up` and `button_down`.
  The `pressed` property is now `button_pressed`.
- [Camera2D](../../classes/class_camera2d.md#class-camera2d)'s `rotating` property was replaced by
  `ignore_rotation`, which has inverted behavior.
- Camera2D's `zoom` property was inverted: higher values are now more zoomed
  in, instead of less.
- [Node](../../classes/class_node.md#class-node)'s `remove_and_skip()` method was removed.
  If you need to reimplement it in a script, you can use the
  [old C++ implementation](https://github.com/godotengine/godot/blob/7936b3cc4c657e4b273b376068f095e1e0e4d82a/scene/main/node.cpp#L1910-L1945)
  as a reference.
- `OS.get_system_time_secs()` should be converted to
  `Time.get_time_dict_from_system()["second"]`.
- [ResourceSaver](../../classes/class_resourcesaver.md#class-resourcesaver)'s `save()` method now has its arguments swapped around
  (`resource: Resource, path: String`). This also applies to
  [ResourceFormatSaver](../../classes/class_resourceformatsaver.md#class-resourceformatsaver)'s `_save()` method.
- A [StreamPeerTCP](../../classes/class_streampeertcp.md#class-streampeertcp) must have `poll()` called on it to update its
  state, instead of relying on `get_status()` automatically polling:
  [GH-59582](https://github.com/godotengine/godot/pull/59582)
- [String](../../classes/class_string.md#class-string)'s `right()` method [has changed behavior](https://github.com/godotengine/godot/pull/36180):
...
- `is_connected_to_host()` was removed from StreamPeerTCP and PacketPeerUDP as
  per [GH-59582](https://github.com/godotengine/godot/pull/59582).
  `get_status()` can be used in StreamPeerTCP instead.
  `is_socket_connected()` can be used in [PacketPeerUDP](../../classes/class_packetpeerudp.md#class-packetpeerudp) instead.
...
- VisualShaderNodeVec4Parameter now takes a [Vector4](../../classes/class_vector4.md#class-vector4) as parameter
  instead of a [Quaternion](../../classes/class_quaternion.md#class-quaternion).
...
| BakedLightmap       | LightmapGI              | See [Using Lightmap global illumination](../3d/global_illumination/using_lightmap_gi.md#doc-using-lightmap-gi).                                                                                             |
| BakedLightmapData   | LightmapGIData          |                                                                                                                                                                                                             |
| BitmapFont          | FontFile                | See [Using Fonts](../ui/gui_using_fonts.md#doc-gui-using-fonts).                                                                                                                                            |
...
| Navigation2D        | Node2D                  | Replaced by [other 2D Navigation nodes](../navigation/navigation_introduction_2d.md#doc-navigation-overview-2d).                                                                                            |
| Navigation3D        | Node3D                  | Replaced by [other 3D Navigation nodes](../navigation/navigation_introduction_3d.md#doc-navigation-overview-3d).                                                                                            |
| OpenSimplexNoise    | FastNoiseLite           | Has different parameters and more noise types such as cellular. No<br/>support for 4D noise as it's absent from the FastNoiseLite library.                                                                  |
...
| ProximityGroup      | Node3D                  | [VisibleOnScreenNotifier3D](../../classes/class_visibleonscreennotifier3d.md#class-visibleonscreennotifier3d) can act as a replacement.                                                                     |
| Portal              | Node3D                  | Portal and room occlusion culling was replaced by raster<br/>[occlusion culling](../3d/occlusion_culling.md#doc-occlusion-culling)<br/>(OccluderInstance3D node), which requires a different setup process. |
| Room                | Node3D                  |                                                                                                                                                                                                             |
| RoomManager         | Node3D                  |                                                                                                                                                                                                             |
| RoomGroup           | Node3D                  |                                                                                                                                                                                                             |
| Occluder            | Node3D                  | Geometry occlusion culling was replaced by raster<br/>[occlusion culling](../3d/occlusion_culling.md#doc-occlusion-culling)<br/>(OccluderInstance3D node), which requires a different setup process.        |
...
[Threading](../performance/using_multiple_threads.md#doc-using-multiple-threads) APIs have changed in 4.0. For
...
#### SEE ALSO
See the [changelog](https://github.com/godotengine/godot/blob/master/CHANGELOG.md)
...
The [editor/renames_map_3_to_4.cpp](https://github.com/godotengine/godot/blob/master/editor/renames_map_3_to_4.cpp)
source file lists all automatic renames performed by the project upgrade tool.
Lines that are commented out refer to API renames that [cannot be performed automatically](#doc-upgrading-to-godot-4-manual-rename).
...
[editor settings folder](../io/data_paths.md#doc-data-paths-editor-data-paths) and copy
...
be ignored by your [version control system](../best_practices/version_control_systems.md#doc-version-control-systems).
