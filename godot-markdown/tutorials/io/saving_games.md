...
[ConfigFile](../../classes/class_configfile.md#class-configfile) class for this purpose.

#### SEE ALSO
You can see how saving and loading works in action using the
[Saving and Loading (Serialization) demo project](https://github.com/godotengine/godot-demo-projects/blob/master/loading/serialization).
...
![image](tutorials/io/img/groups.webp)
...
has the helper class [JSON](../../classes/class_json.md#class-json) to convert between dictionary and string.
...
As covered in the [File system](../scripting/filesystem.md#doc-filesystem) tutorial, we'll need to open a file
so we can write to it or read from it. Now that we have a way to
call our groups and get their relevant data, let's use the class [JSON](../../classes/class_json.md#class-json) to
...
line. Use the [parse](../../classes/class_json.md#class-json-method-parse) method to read the
...
Load parent objects first so they are available for the [add_child()](../../classes/class_node.md#class-node-method-add-child)
call when child objects are loaded. You will also need a way to link
children to parents as the [NodePath](../../classes/class_nodepath.md#class-nodepath) will likely be invalid.
...
a lot of it, [binary serialization](binary_serialization_api.md#doc-binary-serialization-api)
...
[Binary serialization](binary_serialization_api.md#doc-binary-serialization-api) is an alternative
approach for storing game state, and you can use it with the functions
`get_var` and `store_var` of [FileAccess](../../classes/class_fileaccess.md#class-fileaccess).
...
with the [PROPERTY_USAGE_STORAGE](../../classes/class_@globalscope.md#class-globalscope-constant-property-usage-storage)
...
See [PropertyUsageFlags](../../classes/class_@globalscope.md#enum-globalscope-propertyusageflags) for the
