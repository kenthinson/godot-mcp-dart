...
![image](tutorials/3d/img/transforms_euler.webp)

Euler angles were introduced by mathematician Leonhard Euler in the early 1700s.

![image](tutorials/3d/img/transforms_euler_himself.png)
...
![image](tutorials/3d/img/transforms_gimbal.gif)
...
![image](tutorials/3d/img/transforms_interpolate1.gif)

But this does not always have the expected effect when using angles:

![image](tutorials/3d/img/transforms_interpolate2.gif)
...
The result of all this is that you should **not use** the `rotation` property of [Node3D](../../classes/class_node3d.md#class-node3d) nodes in Godot for games. It's there to be used mainly in the editor, for coherence with the 2D engine, and for simple rotations (generally just one axis, or even two in limited cases). As much as you may be tempted, don't use it.

Instead, there is a better way to solve your rotation problems.

## Introducing transforms

Godot uses the [Transform3D](../../classes/class_transform3d.md#class-transform3d) datatype for orientations. Each [Node3D](../../classes/class_node3d.md#class-node3d) node contains a `transform` property which is relative to the parent's transform, if the parent is a Node3D-derived type.

It is also possible to access the world coordinate transform via the `global_transform` property.

A transform has a [Basis](../../classes/class_basis.md#class-basis) (transform.basis sub-property), which consists of three [Vector3](../../classes/class_vector3.md#class-vector3) vectors. These are accessed via the `transform.basis` property and can be accessed directly by `transform.basis.x`, `transform.basis.y`, and `transform.basis.z`. Each vector points in the direction its axis has been rotated, so they effectively describe the node's total rotation. The scale (as long as it's uniform) can also be inferred from the length of the axes. A *basis* can also be interpreted as a 3x3 matrix and used as `transform.basis[x][y]`.
...
![image](tutorials/3d/img/transforms_camera.png)

One way to visualize a transform is to look at an object's 3D gizmo while in "local space" mode.

![image](tutorials/3d/img/transforms_local_space.png)
...
![image](tutorials/3d/img/transforms_gizmo.png)

For more information on the mathematics of vectors and transforms, please read the [Vector math](../math/vector_math.md#doc-vector-math) tutorials.
...
Is the enemy looking at the player? Use the dot product for this (see the [Vector math](../math/vector_math.md#doc-vector-math) tutorial for an explanation of the dot product):
...
The [Quaternion](../../classes/class_quaternion.md#class-quaternion) type reference has more information on the datatype (it
