<a id="doc-using-lightmap-gi"></a>

# Using Lightmap global illumination

Baked lightmaps are a workflow for adding indirect (or fully baked)
lighting to a scene. Unlike the [VoxelGI](using_voxel_gi.md#doc-using-voxel-gi) and
[SDFGI](using_sdfgi.md#doc-using-sdfgi) approaches, baked lightmaps work fine on low-end PCs
...
using [Reflection probes](reflection_probes.md#doc-reflection-probes) together with it on interiors (or using a Sky
...
[Standard Material 3D and ORM Material 3D](../standard_material_3d.md#doc-standard-material-3d) or in custom shaders).
...
See [Which global illumination technique should I use?](introduction_to_global_illumination.md#doc-introduction-to-global-illumination-comparison)
for a comparison of GI techniques available in Godot 4.

## Visual comparison

![LightmapGI disabled.](tutorials/3d/global_illumination/img/gi_none.webp)![LightmapGI enabled (with indirect light baked only).](tutorials/3d/global_illumination/img/gi_lightmap_gi_indirect_only.webp)![LightmapGI enabled (with direct and indirect light baked).](tutorials/3d/global_illumination/img/gi_lightmap_gi_direct_and_indirect.webp)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_comparison.png)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_import.webp)

The **Meshes > Light Baking** option must be set to **Static Lightmaps (VoxelGI/SDFGI/LightmapGI)**:

![image](tutorials/3d/global_illumination/img/lightmap_gi_mesh_import_meshes.webp)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_mesh_menu.webp)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_blender.webp)

Then import the 3D scene normally. Remember you will need to set the texture
size on the mesh after import.

![image](tutorials/3d/global_illumination/img/lightmap_gi_lmsize.webp)
...
#### NOTE
This option is only available for primitive meshes such as [BoxMesh](../../../classes/class_boxmesh.md#class-boxmesh),
[CylinderMesh](../../../classes/class_cylindermesh.md#class-cylindermesh), [PlaneMesh](../../../classes/class_planemesh.md#class-planemesh), etc.
...
[convert a CSG node and its children to a MeshInstance3D](../csg_tools.md#doc-csg-tools-converting-to-mesh-instance-3d).
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_uvchannel.webp)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_scene.webp)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_use.webp)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_bake_mode.webp)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_omnilight_size.png)
...
![image](tutorials/3d/global_illumination/img/lightmap_gi_bake.webp)
...
- **Shadowmask Mode:** If set to a mode other than **None**, the first DirectionalLight3D
  ...
  from the camera. See the [section on shadowmasking](#doc-using-lightmap-gi-shadowmask)
  for further details.
...
- **Use Denoiser:** If enabled, uses a denoising algorithm to make the lightmap
  ...
  [Denoising](#doc-using-lightmap-gi-denoising) for more information.
- **Denoiser Strength:** The strength of denoising step applied to the generated
  ...
  ( does not have a denoiser strength setting).
...
- **Gen Probes > Subdiv:** See [Dynamic objects](#doc-using-lightmap-gi-dynamic-objects).
- **Data > Light Data:** See [Lightmap data](#doc-using-lightmap-gi-data).

<a id="doc-using-lightmap-gi-shadowmask"></a>
...
![Comparison between shadowmask modes](tutorials/3d/global_illumination/img/lightmap_gi_shadowmask.webp)
...
<a id="doc-using-lightmap-gi-denoising"></a>
...
![Comparison between denoising disabled and enabled](tutorials/3d/global_illumination/img/lightmap_gi_denoiser_comparison.webp)
...
![Comparison between JNLM denoiser strength values](tutorials/3d/global_illumination/img/lightmap_gi_denoiser_jnlm_strength.webp)
...
[website](https://www.openimagedenoise.org/downloads.html).
...
![Comparison between JNLM and OIDN denoisers](tutorials/3d/global_illumination/img/lightmap_gi_denoiser_jnlm_vs_oidn.webp)

<a id="doc-using-lightmap-gi-dynamic-objects"></a>
...
- **Manual:** In addition or as an alternative to generating probes
  automatically, you can add light probes manually by adding [LightmapProbe](../../../classes/class_lightmapprobe.md#class-lightmapprobe)
...
<a id="doc-using-lightmap-gi-data"></a>
