<a id="doc-introduction-to-global-illumination"></a>
...
![image](tutorials/3d/global_illumination/img/global_illumination_example.webp)
...
(SSAO) can be enabled in the [environment](../environment_and_post_processing.md#doc-environment-and-post-processing)
...
[Color banding](../3d_rendering_limitations.md#doc-3d-rendering-limitations-color-banding) section in the 3D rendering
...
can be enabled in the [environment](../environment_and_post_processing.md#doc-environment-and-post-processing) settings.
SSR has a significant performance cost (even more so than SSAO), so make sure to disable
it when targeting low-end hardware.

<a id="doc-introduction-to-global-illumination-comparison"></a>
...
![A 3D scene without any form of global illumination (only constant environment lighting). The box and sphere near the camera are both dynamic objects.](tutorials/3d/global_illumination/img/gi_none.webp)

Here's how Godot's various global illumination techniques compare:

- **VoxelGI:** ![average](tutorials/3d/global_illumination/img/score_average.webp) Good reflections and indirect lighting, but beware of leaks.
  - Due to its voxel-based nature, VoxelGI will exhibit light leaks if walls and floors are too thin.
    ...
    this.
    ![VoxelGI in action.](tutorials/3d/global_illumination/img/gi_voxel_gi.webp)
- **SDFGI:** ![average](tutorials/3d/global_illumination/img/score_average.webp) Good reflections and indirect lighting, but beware of leaks and visible cascade shifts.
  - GI level of detail varies depending on the distance
    ...
    ![SDFGI in action.](tutorials/3d/global_illumination/img/gi_sdfgi.webp)
- **Screen-space indirect lighting (SSIL):** ![average](tutorials/3d/global_illumination/img/score_average.webp) Good *secondary* source of indirect lighting, but no reflections.
  - SSIL is designed to be used as a complement to another GI technique such as
    ...
    ![SSIL in action (without any other GI technique). Notice the emissive lighting around the yellow box.](tutorials/3d/global_illumination/img/gi_ssil_only.webp)
- **LightmapGI:** ![good](tutorials/3d/global_illumination/img/score_good.webp) Excellent indirect lighting, decent reflections (optional).
  - This is the only technique where the number of light bounces
    ...
    ![LightmapGI in action. Only indirect lighting is baked here, but direct light can also be baked.](tutorials/3d/global_illumination/img/gi_lightmap_gi_indirect_only.webp)
- **ReflectionProbe:** ![average](tutorials/3d/global_illumination/img/score_average.webp) Good reflections, but poor indirect lighting.
  - Indirect lighting can be disabled, set to a constant color spread throughout
    ...
    ![ReflectionProbe in action (without any other GI technique). Notice the reflective sphere.](tutorials/3d/global_illumination/img/gi_none_reflection_probe.webp)

### Real-time ability

- **VoxelGI:** ![good](tutorials/3d/global_illumination/img/score_good.webp) Fully real-time.
...
- **SDFGI:** ![average](tutorials/3d/global_illumination/img/score_average.webp) Semi-real-time.
  - Cascades are generated in real-time, making SDFGI
    ...
    slow-moving objects.
- **Screen-space indirect lighting (SSIL):** ![good](tutorials/3d/global_illumination/img/score_good.webp) Fully real-time.
...
- **LightmapGI:** ![bad](tutorials/3d/global_illumination/img/score_bad.webp) Baked, and therefore not real-time.
  - Both indirect lighting and SH reflections are baked and can't be changed at
    runtime. Real-time GI must be
    [simulated via other means](faking_global_illumination.md#doc-faking-global-illumination),
...
- **ReflectionProbe:** ![average](tutorials/3d/global_illumination/img/score_average.webp) Optionally real-time.
...
- For desktop games, it's a good idea to start with [SDFGI](using_sdfgi.md#doc-using-sdfgi)
  ...
  graphics, consider adding an option to disable SDFGI or [VoxelGI](using_voxel_gi.md#doc-using-voxel-gi) in your game's settings. SDFGI can be disabled in the
...
- For mobile games, [LightmapGI](using_lightmap_gi.md#doc-using-lightmap-gi) and
  [ReflectionProbes](reflection_probes.md#doc-reflection-probes) are the only supported options.
  See also [Alternatives to GI techniques](#doc-introduction-to-global-illumination-alternatives).

#### SEE ALSO
You can compare global illumination techniques in action using the
[Global Illumination demo project](https://github.com/godotengine/godot-demo-projects/tree/master/3d/global_illumination).

<a id="doc-introduction-to-global-illumination-gi-mode-recommendations"></a>
...
<a id="doc-introduction-to-global-illumination-alternatives"></a>

## Alternatives to GI techniques

If none of the GI techniques mentioned above fits, it's still possible to
[simulate GI by placing additional lights manually](faking_global_illumination.md#doc-faking-global-illumination).
