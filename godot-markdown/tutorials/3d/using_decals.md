...
properties to full-blown  materials.
...
[3D decals demo](https://github.com/godotengine/godot-demo-projects/tree/master/3d/decals).

If you're looking to write arbitrary 3D text on top of a surface, use
[3D text](3d_text.md#doc-3d-text) placed close to a surface instead of a Decal node.
...
![Dirt added on top of level geometry using decals](tutorials/3d/img/decals_dirt.webp)
...
![Blob shadow under object comparison](tutorials/3d/img/decals_blob_shadow.webp)
...
- **Normal Fade:** Fades the Decal if the angle between the Decal's
   and the target surface becomes too large.
...
- **Lower Fade:** The curve over which the decal will fade as the surface gets
  further from the center of the  (away
...
By default, decals are ordered based on the size of their  and the distance to the camera. AABBs that are
...
![VisualInstance3D Sorting Offset comparison on Decals](tutorials/3d/img/decals_sorting_offset.webp)
...
[Rendering > Textures > Decals > Filter](../../classes/class_projectsettings.md#class-projectsettings-property-rendering-textures-decals-filter)
...
an omni light, a spot light, a [decal](#doc-using-decals) or a
