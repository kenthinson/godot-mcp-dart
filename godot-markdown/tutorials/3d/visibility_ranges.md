<a id="doc-visibility-ranges"></a>

# Visibility ranges (HLOD)

Along with [Mesh level of detail (LOD)](mesh_lod.md#doc-mesh-lod) and [Occlusion culling](occlusion_culling.md#doc-occlusion-culling),
...
[Mesh level of detail (LOD)](mesh_lod.md#doc-mesh-lod) instead.
...
for artist-controlled , but also
...
to use different node types as part of a  system.
...
The benefit of  over a traditional
 system is its hierarchical nature. A single larger
...
<a id="doc-visibility-ranges-fade-mode"></a>
...
- **Dependencies:** Uses alpha blending to smoothly fade between LOD levels. The
  ...
  [Visibility parent](#doc-visibility-ranges-visibility-parent). It acts
  the same as **Self** if visibility ranges are used to perform non-hierarchical
  LOD.

<a id="doc-visibility-ranges-visibility-parent"></a>

### Visibility parent

The **Visibility Parent** property makes it easier to set up
. It allows automatically hiding
...
[GeometryInstance3D](../../classes/class_geometryinstance3d.md#class-geometryinstance3d).
...
This is done to minimize visible pop-in. Depending on your  setup, you may want to try both the **Self** and
**Dependencies** [fade modes](#doc-visibility-ranges-fade-mode).
...
- No visual glitches due to
  [transparency sorting issues](3d_rendering_limitations.md#doc-3d-rendering-limitations-transparency-sorting)
