...
[Occlusion Culling and Mesh LOD demo project](https://github.com/godotengine/godot-demo-projects/tree/master/3d/occlusion_culling_mesh_lod).
...
![Example scene with an occlusion culling-friendly layout](tutorials/3d/img/occlusion_culling_scene_example.png)
...
![Example scene with occlusion culling disabled (wireframe)](tutorials/3d/img/occlusion_culling_disabled.png)
...
![Example scene with occlusion culling enabled (wireframe)](tutorials/3d/img/occlusion_culling_enabled.png)
...
this with [Mesh level of detail (LOD)](mesh_lod.md#doc-mesh-lod) and [Visibility ranges (HLOD)](visibility_ranges.md#doc-visibility-ranges) to further improve
performance gains.
...
the software raytracing library [Embree](https://github.com/embree/embree).

The engine then uses this low-resolution buffer to test the occludee's
 against the occluder shapes.
The occludee's  must be *fully occluded*
...
<a id="doc-occlusion-culling-baking"></a>
...
[converted to a MeshInstance3D](csg_tools.md#doc-csg-tools-converting-to-mesh-instance-3d)
...
<a id="doc-occlusion-culling-preview"></a>
...
the nodes themselves to move in global space, therefore requiring the  to be rebuilt.
...
[My occludee is being culled when it shouldn't be](#doc-occlusion-culling-troubleshooting-false-negative).
...
<a id="doc-occlusion-culling-troubleshooting-false-negative"></a>
...
[occlusion culling debug draw mode](#doc-occlusion-culling-preview) to look
for occluder shapes that shouldn't be present, then
[adjust the bake cull mask accordingly](#doc-occlusion-culling-baking).
