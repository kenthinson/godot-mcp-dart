...
performance in a 3D project, along with [Occlusion culling](occlusion_culling.md#doc-occlusion-culling).
...
The [meshoptimizer](https://meshoptimizer.org/) library is used for LOD mesh
...
![From most detailed (left) to least detailed (right), shaded view](tutorials/3d/img/mesh_lod_comparison_shaded.png)

Here's the same image with wireframe rendering to make the decimation easier to see:

![From most detailed (left) to least detailed (right), wireframe view](tutorials/3d/img/mesh_lod_comparison_wireframe.png)

#### SEE ALSO
If you need to manually configure level of detail with artist-created meshes,
use [Visibility ranges (HLOD)](visibility_ranges.md#doc-visibility-ranges) instead of automatic mesh LOD.
...
![Changing the import type on an OBJ file in the Import dock](tutorials/3d/img/mesh_lod_obj_import.png)
...
See [Importing 3D scenes](../assets_pipeline/importing_3d_scenes/import_configuration.md#doc-importing-3d-scenes-using-the-import-dock)
...
![Disabling mesh LOD in the 3D viewport's top-left menu](tutorials/3d/img/mesh_lod_disable_lod.png)
...
As a result, unlike [Visibility ranges (HLOD)](visibility_ranges.md#doc-visibility-ranges), you don't need to do
