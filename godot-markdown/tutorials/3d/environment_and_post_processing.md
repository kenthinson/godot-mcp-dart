<a id="doc-environment-and-post-processing"></a>
...
The [Environment](../../classes/class_environment.md#class-environment) resource stores all the information required for
...
![image](tutorials/3d/img/environment_camera.webp)
...
![image](tutorials/3d/img/environment_world.webp)
...
![image](tutorials/3d/img/environment_preview_sun_sky_toggle.webp)
...
![image](tutorials/3d/img/environment_preview_sun_sky_dialog.webp)
...
The [CameraAttributes](../../classes/class_cameraattributes.md#class-cameraattributes) resource stores exposure and depth of field
...
#### NOTE
Using a [CameraAttributesPhysical](../../classes/class_cameraattributesphysical.md#class-cameraattributesphysical) on a Camera3D node will lock
...
![image](tutorials/3d/img/environment_background1.webp)
...
- **Canvas** displays the 2D scene as a background to the 3D scene. This can be used
  ...
  [glow in 2D](#doc-environment-and-post-processing-using-glow-in-2d).
- **Keep** does not draw any sky, keeping what was present on previous frames
  ...
  - **Camera Feed** displays a [CameraFeed](../../classes/class_camerafeed.md#class-camerafeed) from a physical camera as the
...
You can find freely licensed HDRIs on [Poly Haven](https://polyhaven.com/hdris).
...
create a custom [sky shader](../shaders/shader_reference/sky_shader.md#doc-sky-shader).
...
![image](tutorials/3d/img/environment_ambient2.webp)
...
generate ambient light from [ReflectionProbe](global_illumination/reflection_probes.md#doc-reflection-probes),
[VoxelGI](global_illumination/using_voxel_gi.md#doc-using-voxel-gi) or [SDFGI](global_illumination/using_sdfgi.md#doc-using-sdfgi), as these
...
![image](tutorials/3d/img/environment_ambient_comparison.webp)
...
[Reflection probes](global_illumination/reflection_probes.md#doc-reflection-probes), they may not provide enough detail for all
...
![image](tutorials/3d/img/environment_ssr.webp)
...
light (on the left). If you want to force
 to work with direct light too,
use the **Light Affect** parameter. Even though this is not physically correct,
some artists like how it looks.

 looks best when combined with a
real source of indirect light, like VoxelGI:

![image](tutorials/3d/img/environment_ssao2.webp)

Tweaking  is possible with several
parameters:

![image](tutorials/3d/img/environment_ssao_parameters.webp)
...
- **Intensity:** The primary screen-space ambient occlusion intensity. Acts as a
  ...
  Since  is a screen-space effect,
  it's recommended to remain conservative with this value.
   that is too strong can be
...
<a id="doc-environment-and-post-processing-ssil"></a>
...
Mobile or Compatibility.*
 ...
 provides indirect lighting for
...
offered by other 3D engines. 
can be combined with  and/or
 for greater visual quality
(at the cost of performance).

Tweaking  is possible with several parameters:
...
![image](tutorials/3d/img/environment_ssil.webp)
...
illumination for off-screen elements (unlike ).

#### SEE ALSO
See [Signed distance field global illumination (SDFGI)](global_illumination/using_sdfgi.md#doc-using-sdfgi) for instructions on setting up this global
illumination technique.

![image](tutorials/3d/img/environment_sdfgi.webp)

<a id="doc-environment-and-post-processing-glow"></a>
...
![image](tutorials/3d/img/environment_glow1.webp)
...
![image](tutorials/3d/img/environment_glow_threshold.webp)
...
![image](tutorials/3d/img/environment_glow_bloom.webp)
...
- **Replace** can be used to
  [blur the whole screen](#doc-environment-and-post-processing-using-glow-to-blur-the-screen)
...
![image](tutorials/3d/img/environment_glow_layers.webp)

The real strength of this system, though, is to combine levels to create more
interesting glow patterns:

![image](tutorials/3d/img/environment_glow_layers2.webp)
...
![image](tutorials/3d/img/environment_glow_map.webp)
...
![image](tutorials/3d/img/environment_and_post_processing_glow_scale_mode.webp)

<a id="doc-environment-and-post-processing-using-glow-in-2d"></a>
...
- Since Godot 4.2, you can enable HDR for 2D rendering when using the Forward+
  ...
  - To enable HDR in 2D, open the Project Settings, enable
    [Rendering > Viewport > HDR 2D](../../classes/class_projectsettings.md#class-projectsettings-property-rendering-viewport-hdr-2d)
...
- If you want to maximize performance, you can leave HDR disabled for 2D
  ...
  - Enable glow, set the environment background mode to **Canvas** then decrease
    **Glow HDR Threshold** so that pixels that are not overbright will still
    ...
    [CanvasLayer](../../classes/class_canvaslayer.md#class-canvaslayer) node. You can control which layers are affected by
    glow using the **Background > Canvas Max Layer** property of the Environment
    resource.

![Example of using glow in a 2D scene](tutorials/3d/img/environment_and_post_processing_glow_in_2d.webp)
...
<a id="doc-environment-and-post-processing-using-glow-to-blur-the-screen"></a>
...
when using the Canvas background mode, make them children of a [CanvasLayer](../../classes/class_canvaslayer.md#class-canvaslayer)
...
![Example of using glow to blur the 2D rendering in the menu's background](tutorials/3d/img/environment_and_post_processing_glow_blur.webp)

### Fog

#### NOTE
This section refers to non-volumetric fog only.
It is possible to use both non-volumetric fog and [Volumetric fog and fog volumes](volumetric_fog.md#doc-volumetric-fog)
at the same time.
...
![image](tutorials/3d/img/environment_fog_depth_height.webp)
...
higher density levels. See [Color banding](3d_rendering_limitations.md#doc-3d-rendering-limitations-color-banding)
...
#### SEE ALSO
See [Volumetric fog and fog volumes](volumetric_fog.md#doc-volumetric-fog) for documentation on setting up volumetric fog.
...
![image](tutorials/3d/img/environment_adjustments.webp)
...
![image](tutorials/3d/img/environment_adjustments_bcs.webp)
...
![image](tutorials/3d/img/environment_adjustments_default_gradient.webp)

But creating custom ones will allow to map each channel to a different color:

![image](tutorials/3d/img/environment_adjustments_custom_gradient.webp)
...
![image](tutorials/3d/img/environment_adjustments_3d_lut_import.webp)
...
![image](tutorials/3d/img/environment_adjustments_3d_lut_template.webp)
...
![image](tutorials/3d/img/environment_adjustments_3d_lut_comparison.webp)
...
![image](tutorials/3d/img/environment_dof_far.webp)
...
![image](tutorials/3d/img/environment_dof_near.webp)
...
["tilt shift" effect](https://en.wikipedia.org/wiki/Miniature_faking).

![image](tutorials/3d/img/environment_mixed_blur.webp)
...
![image](tutorials/3d/img/environment_hdr_autoexp.webp)
...
By combining Auto Exposure with [Glow](#doc-environment-and-post-processing-glow)
...
#### SEE ALSO
See [Physical light and camera units](physical_light_and_camera_units.md#doc-physical-light-and-camera-units) if you wish to use real world
