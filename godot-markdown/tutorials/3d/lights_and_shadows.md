...
- Ambient light in the [Environment](../../classes/class_environment.md#class-environment) or
  [Reflection probes](global_illumination/reflection_probes.md#doc-reflection-probes).
- Global illumination ([LightmapGI](global_illumination/using_lightmap_gi.md#doc-using-lightmap-gi),
  [VoxelGI](global_illumination/using_voxel_gi.md#doc-using-voxel-gi) or [SDFGI](global_illumination/using_sdfgi.md#doc-using-sdfgi)).

The emission color is a material property. You can read more about it
in the [Standard Material 3D and ORM Material 3D](standard_material_3d.md#doc-standard-material-3d) tutorial.

#### SEE ALSO
You can compare various types of lights in action using the
[3D Lights and Shadows demo project](https://github.com/godotengine/godot-demo-projects/tree/master/3d/lights_and_shadows).

## Light nodes

There are three types of light nodes: [DirectionalLight3D](../../classes/class_directionallight3d.md#class-directionallight3d),
[OmniLight3D](../../classes/class_omnilight3d.md#class-omnilight3d) and [SpotLight3D](../../classes/class_spotlight3d.md#class-spotlight3d). Let's take a look at the common
parameters for lights:

![image](tutorials/3d/img/light_params.png)

Each property has a specific function:

- **Color:** Base color for emitted light.
- **Energy:** Energy multiplier. This is useful for saturating lights or working with [High dynamic range lighting](high_dynamic_range.md#doc-high-dynamic-range).
- **Indirect Energy:** Secondary multiplier used with indirect light (light bounces). This works with [Using Lightmap global illumination](global_illumination/using_lightmap_gi.md#doc-using-lightmap-gi), VoxelGI or SDFGI.
...
- **Bake Mode:** Sets the bake mode for the light. See [Using Lightmap global illumination](global_illumination/using_lightmap_gi.md#doc-using-lightmap-gi).
...
an omni light, a spot light, a [decal](using_decals.md#doc-using-decals) or a
[reflection probe](global_illumination/reflection_probes.md#doc-reflection-probes). This limit can be increased by adjusting
[Max Clustered Elements](../../classes/class_projectsettings.md#class-projectsettings-property-rendering-limits-cluster-builder-max-clustered-elements)
...
[Max Renderable Elements](../../classes/class_projectsettings.md#class-projectsettings-property-rendering-limits-opengl-max-renderable-elements)
and/or [Max Lights per Object](../../classes/class_projectsettings.md#class-projectsettings-property-rendering-limits-opengl-max-lights-per-object)
...
consider using [baked lightmaps](global_illumination/using_lightmap_gi.md#doc-using-lightmap-gi) with lights' bake
...
[global illumination](global_illumination/introduction_to_global_illumination.md#doc-introduction-to-global-illumination) technique
...
- **Blur:** Multiplies the shadow blur radius for this light. This works with
  ...
  **Angular Distance** or **Size** greater than `0.0`). Higher values result
  ...
  See also [Shadow filter mode](#doc-lights-and-shadows-shadow-filter-mode).
...
![image](tutorials/3d/img/lights_and_shadows_blur.webp)
...
![image](tutorials/3d/img/lights_and_shadows_acne.webp)
...
![image](tutorials/3d/img/lights_and_shadows_peter_panning.webp)
...
[increasing the shadow map resolution](#doc-lights-and-shadows-balancing-performance-and-quality),
...
![image](tutorials/3d/img/light_directional.png)
...
recommendations in [PCSS recommendations](#doc-lights-and-shadows-pcss-recommendations) if setting
...
![image](tutorials/3d/img/lights_and_shadows_pssm_explained.webp)

With this, shadows become more detailed:

![image](tutorials/3d/img/lights_and_shadows_directional_mode.webp)

To control PSSM, a number of parameters are exposed:

![image](tutorials/3d/img/lights_and_shadows_directional_shadow_params.webp)
...
![image](tutorials/3d/img/blend_splits.png)
...
![image](tutorials/3d/img/light_omni.png)
...
[PCSS recommendations](#doc-lights-and-shadows-pcss-recommendations) if setting this value above
`0.0` on lights with shadows enabled.

![image](tutorials/3d/img/lights_and_shadows_pcss.webp)
...
![image](tutorials/3d/img/lights_and_shadows_dual_parabolid_vs_cubemap.webp)
...
to [PanoramaSkyMaterial](../../classes/class_panoramaskymaterial.md#class-panoramaskymaterial) textures.

With the projector texture below, the following result is obtained:

![image](tutorials/3d/img/lights_and_shadows_omni_projector_example.webp)![image](tutorials/3d/img/lights_and_shadows_omni_projector.webp)
...
[decal](using_decals.md#doc-using-decals).

With the projector texture below, the following result is obtained:

![image](tutorials/3d/img/lights_and_shadows_spot_projector_example.webp)![image](tutorials/3d/img/lights_and_shadows_spot_projector.webp)
...
<a id="doc-lights-and-shadows-shadow-atlas"></a>
...
![image](tutorials/3d/img/lights_and_shadows_shadow_quadrants.webp)

Each quadrant can be subdivided to allocate any number of shadow maps; the following is the default subdivision:

![image](tutorials/3d/img/lights_and_shadows_shadow_quadrants2.webp)
...
<a id="doc-lights-and-shadows-balancing-performance-and-quality"></a>
...
appropriate [RenderingServer](../../classes/class_renderingserver.md#class-renderingserver) methods.

Positional (omni/spot) shadow quality settings can be changed at runtime on the
root [Viewport](../../classes/class_viewport.md#class-viewport).
...
[shadow atlas](#doc-lights-and-shadows-shadow-atlas) quadrants to contain
fewer shadows. This will allow each shadow to be rendered at a higher resolution.

<a id="doc-lights-and-shadows-shadow-filter-mode"></a>
...
[Temporal antialiasing (TAA)](3d_antialiasing.md#doc-3d-antialiasing-taa), [AMD FidelityFX Super Resolution 2.2 (FSR2)](3d_antialiasing.md#doc-3d-antialiasing-fsr2),
[Fast approximate antialiasing (FXAA)](3d_antialiasing.md#doc-3d-antialiasing-fxaa), or increase the shadow filter quality to
...
![image](tutorials/3d/img/lights_and_shadows_filter_quality.webp)
...
<a id="doc-lights-and-shadows-pcss-recommendations"></a>
