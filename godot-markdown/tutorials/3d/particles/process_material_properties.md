<a id="doc-process-material-properties"></a>

# Process material properties

![ParticleProcessMaterial properties](tutorials/3d/particles/img/particle_minmaxcurve.webp)
...
For information on how to use it in a 2D Scene see [ParticleProcessMaterial 2D Usage](../../2d/particle_process_material_2d.md#doc-particle-process-material-2d).
...
the same amount of time, set by the [Lifetime](properties.md#doc-3d-particles-properties-time) property. A value of `1` means
...
property is the same as setting the [Transform Align](properties.md#doc-3d-particles-properties-draw) property to
`Y to Velocity`.

The `Rotate Y` property works with the properties in the [Angle](#angle) and
[Angular Velocity](#angular-velocity) groups to control particle rotation. `Rotate Y`
has to be enabled if you want to apply any rotation to particles. The exception to this
is any particle that uses the [Standard Material](../standard_material_3d.md#doc-standard-material-3d)
...
determined by the [Local Coords](properties.md#doc-3d-particles-properties-draw) property.
...
<a id="doc-process-material-properties-shapes"></a>
...
![Simple particle emission shapes](tutorials/3d/particles/img/particle_shapes_simple.webp)![Ring-shaped particle system](tutorials/3d/particles/img/particle_ring.webp)
...
[Complex emission shapes](complex_shapes.md#doc-3d-particles-complex-shapes) section for a detailed
explanation of how to set these up.

## Angle

The `Angle` property controls a particle's starting rotation [as described above](#process-material-properties).
In order to have an actual effect on the particle, you have to enable one of two properties: [Rotate Y](#particle-flags)
rotates the particle around the particle system's Y-axis. The `Billboard` property in
the [Standard Material](../standard_material_3d.md#doc-standard-material-3d), if it is set to `Particle Billboard`, rotates
...
![Different values for particle direction](tutorials/3d/particles/img/particle_direction.webp)
...
![Different values for particle spread](tutorials/3d/particles/img/particle_spread.webp)
...
values for either of these properties [as described above](#process-material-properties), the
...
![Different values for particle gravity](tutorials/3d/particles/img/particle_gravity.webp)

## Angular velocity

`Angular Velocity` controls a particle's speed of rotation [as described above](#process-material-properties).
You can reverse the direction by using negative numbers for `Velocity Min` or `Velocity Max`. Like the
[Angle](#angle) property, the rotation will only be visible if the [Rotate Y](#particle-flags) flag is set
or the `Particle Billboard` mode is selected in the [Standard Material](../standard_material_3d.md#doc-standard-material-3d).
...
change the speed of movement over a particle's lifetime [as described above](#process-material-properties).
...
![Different values for particle linear acceleration](tutorials/3d/particles/img/particle_accel_linear.webp)
...
![Different values for particle radial acceleration](tutorials/3d/particles/img/particle_accel_radial.webp)

## Tangential acceleration

![Tangents on a circle](tutorials/3d/particles/img/particle_tangent.webp)
...
![Tangential acceleration from above](tutorials/3d/particles/img/particle_accel_tangent.webp)
...
If you want the particle system to interact with [particle attractors](attractors.md#doc-3d-particles-attractors),
...
`Scale` controls a particle's size [as described above](#process-material-properties). You can set
...
billboards, the `Keep Size` property on the [Standard Material](../standard_material_3d.md#doc-standard-material-3d)
...
`Use As Albedo` property in the `Vertex Color` group of the [Standard Material](../standard_material_3d.md#doc-standard-material-3d)
is enabled. This property is multiplied with color coming from the particle material's
own `Color` or `Texture` property.

![Particle color ramp](tutorials/3d/particles/img/particle_ramp.webp)
...
property controls the range of available hues [as described above](#process-material-properties).
...
![Different values for hue variation](tutorials/3d/particles/img/particle_hue.webp)

<a id="doc-process-material-properties-animation"></a>

## Animation

The `Animation` property group controls the behavior of sprite
sheet animations in the particle's [Standard Material](../standard_material_3d.md#doc-standard-material-3d).
The `Min`, `Max`, and `Curve` values work [as described above](#process-material-properties).
...
![A sprite sheet](tutorials/3d/particles/img/particle_sprite.webp)
1. Import a sprite sheet texture into the engine. If you don't have one at hand, you can download the [`high-res version of the example image`](img/particle_sprite_smoke.webp).
...
![Animated particles](tutorials/3d/particles/img/particle_animate.webp)
...
![Animated particles lifetimes](tutorials/3d/particles/img/particle_animate_lifetime.webp)
...
<a id="doc-process-material-properties-turbulence"></a>
...
[particle turbulence](turbulence.md#doc-3d-particles-turbulence).

<a id="doc-process-material-properties-subemitter"></a>
...
If the `Use Scale` property is enabled, the [collision base size](properties.md#doc-3d-particles-properties-collision)
...
You can learn more about particle collisions in the [Collisions](collision.md#doc-3d-particles-collision)
section in this manual.

# Sub-emitter

![Sub-emitter modes](tutorials/3d/particles/img/particle_sub_mode.webp)
...
See the [Sub-emitters](subemitters.md#doc-3d-particles-subemitters) section in this manual for a detailed explanation of how
...
the current ParticleProcessMaterial to a [ShaderMaterial](../../../classes/class_shadermaterial.md#class-shadermaterial). Existing properties
