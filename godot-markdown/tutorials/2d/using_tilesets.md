...
benefits to using [TileMapLayer](../../classes/class_tilemaplayer.md#class-tilemaplayer) nodes to design your
levels. First, they let you draw a layout by "painting" tiles onto a grid,
which is much faster than placing individual [Sprite2D](../../classes/class_sprite2d.md#class-sprite2d) nodes one by one. Second, they allow for larger levels
...
TileSet, you will be able to place them [using the TileMap editor](using_tilemaps.md#doc-using-tilemaps).
...
<a id="doc-creating-tilesets-using-tilesheet"></a>
...
![Tilesheet example with 64×64 tiles](tutorials/2d/img/using_tilesets_kenney_abstract_platformer_tile_sheet.webp)

Create a new **TileMapLayer** node, then select it and create a new TileSet resource in the inspector:

![Creating a new TileSet resource within the TileMapLayer node](tutorials/2d/img/using_tilesets_create_new_tileset.webp)
...
![Setting the tile size to 64×64 to match the example tilesheet](tutorials/2d/img/using_tilesets_specify_size_then_edit.webp)
...
![Automatically creating tiles based on tilesheet image content](tutorials/2d/img/using_tilesets_create_tiles_automatically.webp)
...
![Using the Eraser tool to remove unwanted tiles from the TileSet atlas](tutorials/2d/img/using_tilesets_eraser_tool.webp)
...
![Adjusting TileSet atlas properties in the dedicated inspector (part of the TileSet panel)](tutorials/2d/img/using_tilesets_properties.webp)
...
![Recreating tiles automatically after changing atlas properties](tutorials/2d/img/using_tilesets_recreate_tiles_automatically.webp)
...
[use atlases instead](#doc-creating-tilesets-using-tilesheet).
...
![Creating a scenes collection in the TileSet editor](tutorials/2d/img/using_tilesets_creating_scene_collection.webp)
...
![Creating a scene tile after selecting the scenes collection in the TileSet editor](tutorials/2d/img/using_tilesets_scene_collection_create_scene_tile.webp)

Select this scene slot in the right column, then use **Quick Load** (or
**Load**) to load the scene file containing the particles:

![Creating a scene slot, then loading a scene file into it in the TileSet editor](tutorials/2d/img/using_tilesets_adding_scene_tile.webp)
...
![Opening the atlas merging tool after creating multiple atlases](tutorials/2d/img/using_tilesets_open_atlas_merging_tool.webp)
...
![Using the atlas merging tool dialog](tutorials/2d/img/using_tilesets_atlas_merging_tool_dialog.webp)
...
If you use [2D navigation](../navigation/navigation_introduction_2d.md#doc-navigation-overview-2d), you'll also need
to define navigation polygons for tiles to generate a navigation mesh that
agents can use for pathfinding.

Lastly, if you use [2D lights and shadows](2d_lights_and_shadows.md#doc-2d-lights-and-shadows) or GPUParticles2D, you may
...
![Creating a physics layer in the TileSet resource inspector (within the TileMapLayer node)](tutorials/2d/img/using_tilesets_create_physics_layer.webp)

If you also need navigation support, now is a good time to create a navigation layer:

![Creating a navigation layer in the TileSet resource inspector (within the TileMapLayer node)](tutorials/2d/img/using_tilesets_create_navigation_layer.webp)

If you need support for light polygon occluders, now is a good time to create an occlusion layer:

![Creating an occlusion layer in the TileSet resource inspector (within the TileMapLayer node)](tutorials/2d/img/using_tilesets_create_occlusion_layer.webp)
...
![Opening the collision editor while in Select mode](tutorials/2d/img/using_tilesets_selecting_collision_editor.webp)
...
![Using default rectangle collision shape by pressing :kbd:`F`](tutorials/2d/img/using_tilesets_using_default_rectangle_collision.webp)
...
![Creating a triangle collision shape by right-clicking one of the corners to remove it](tutorials/2d/img/using_tilesets_creating_triangle_collision.webp)

You can also use the rectangle as a base for more complex shapes by adding more points:

![Drawing a custom collision for a complex tile shape](tutorials/2d/img/using_tilesets_drawing_custom_collision.webp)
...
alternative tile](#doc-using-tilesets-creating-alternative-tiles) and changing
the custom data for the alternative tile only.

![Creating a custom data layer in the TileSet resource inspector (within the TileMapLayer node)](tutorials/2d/img/using_tilesets_create_custom_data_layer.webp)![Example of configured custom data layers with game-specific properties](tutorials/2d/img/using_tilesets_custom_data_layers_example.webp)
...
![Editing custom data in the TileSet editor while in Select mode](tutorials/2d/img/using_tilesets_edit_custom_data.webp)

[Tile property painting](#doc-using-tilemaps-using-tile-property-painting)
can also be used for custom data:

![Assigning custom data in the TileSet editor using tile property painting](tutorials/2d/img/using_tilesets_paint_custom_data.webp)

<a id="doc-using-tilesets-creating-terrain-sets"></a>
...
![Creating a terrain set in the TileSet resource inspector (within the TileMapLayer node)](tutorials/2d/img/using_tilesets_create_terrain_set.webp)

After creating a terrain set, you **must** create one or more terrains *within* the terrain set:

![Creating a terrain within the terrain set](tutorials/2d/img/using_tilesets_create_terrain.webp)
...
![Configuring terrain on a single tile in the TileSet editor's Select mode](tutorials/2d/img/using_tilesets_configure_terrain_on_tile.webp)
...
![Configuring terrain peering bits on a single tile in the TileSet editor's Select mode](tutorials/2d/img/using_tilesets_configure_terrain_peering_bits.webp)

An example configuration for a full tilesheet may look as follows:

![Example full tilesheet for a sidescrolling game](tutorials/2d/img/using_tilesets_terrain_example_tilesheet.webp)![Example full tilesheet for a sidescrolling game with terrain peering bits visible](tutorials/2d/img/using_tilesets_terrain_example_tilesheet_configuration.webp)

<a id="doc-using-tilemaps-assigning-properties-to-multiple-tiles"></a>
...
![Selecting multiple tiles using the Select mode, then applying properties](tutorials/2d/img/using_tilesets_select_and_set_tile_properties.webp)

<a id="doc-using-tilemaps-using-tile-property-painting"></a>
...
![Painting tile properties using the TileSet editor](tutorials/2d/img/using_tilesets_paint_tile_properties.webp)
...
![Painting a collision polygon, then left-clicking tiles to apply it](tutorials/2d/img/using_tilesets_paint_tile_properties_collision.webp)

<a id="doc-using-tilesets-creating-alternative-tiles"></a>
...
![Creating an alternative tile by right-clicking a base tile in the TileSet editor](tutorials/2d/img/using_tilesets_create_alternative_tile.webp)
...
![Configuring an alternative tile after clicking it in the TileSet editor](tutorials/2d/img/using_tilesets_configure_alternative_tile.webp)
