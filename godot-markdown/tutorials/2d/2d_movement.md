...
We'll use [CharacterBody2D](../../classes/class_characterbody2d.md#class-characterbody2d) for these examples,
but the principles will apply to other node types (Area2D, RigidBody2D) as well.

<a id="doc-2d-movement-setup"></a>
...
input actions (see [InputEvent](../inputs/inputevent.md#doc-inputevent) for details):

![image](tutorials/2d/img/movement_inputs.webp)
...
In the `get_input()` function, we use [Input](../../classes/class_input.md#class-input) `get_vector()` to check for the
...
[Setup](#doc-2d-movement-setup) part of this tutorial.
...
Here we're using the [Node2D](../../classes/class_node2d.md#class-node2d) `look_at()` method to
