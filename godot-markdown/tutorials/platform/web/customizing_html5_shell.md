<a id="doc-customizing-html5-shell"></a>
...
JavaScript code that calls the [`Engine()`](html5_shell_classref.md#Engine) class.

The only required placeholders are:

- `$GODOT_URL`:
  The name of the main JavaScript file, which provides the [`Engine()`](html5_shell_classref.md#Engine) class required
...
- `$GODOT_CONFIG`:
  ...
  See [`EngineConfig`](html5_shell_classref.md#EngineConfig) for the full list of overrides.
...
- `$GODOT_PROJECT_NAME`:
  ...
  [Name](../../../classes/class_projectsettings.md#class-projectsettings-property-application-config-name) setting
...
- `$GODOT_SPLASH`:
  ...
  [Image](../../../classes/class_projectsettings.md#class-projectsettings-property-application-boot-splash-image) setting
  in **Project Settings > Application > Boot Splash**.
- `$GODOT_SPLASH_COLOR`
  The splash screen background color as defined in the
  [BG Color](../../../classes/class_projectsettings.md#class-projectsettings-property-application-boot-splash-bg-color) setting
...
- `$GODOT_SPLASH_CLASSES`:
  ...
  - [Show Image](../../../classes/class_projectsettings.md#class-projectsettings-property-application-boot-splash-show-image):
    `show-image--true`, `show-image--false`
  - [Stretch Mode](../../../classes/class_projectsettings.md#class-projectsettings-property-application-boot-splash-stretch-mode):
    `fullsize--true` (if **not** Disabled), `fullsize--false`
  - [Use Filter](../../../classes/class_projectsettings.md#class-projectsettings-property-application-boot-splash-use-filter):
...
![image](tutorials/platform/web/img/html5_export_options.png)
...
See the [HTML5 shell class reference](html5_shell_classref.md#doc-html5-shell-classref), for the full list of methods and options available.
...
However, in the simplest case all you need to do is to create an instance of the [`Engine()`](html5_shell_classref.md#Engine)
class with the exported configuration, and then call the [`engine.startGame`](html5_shell_classref.md#Engine.prototype.startGame) method
optionally overriding any [`EngineConfig`](html5_shell_classref.md#EngineConfig) parameters.
...
It uses the given configuration to load the engine. The [`engine.startGame`](html5_shell_classref.md#Engine.prototype.startGame)
...
the [`engine.start`](html5_shell_classref.md#Engine.prototype.start) method can be used instead. Note, that this method do not
automatically preload the `pck` file, so you will probably want to manually preload it
(and any other extra file) via the [`engine.preloadFile`](html5_shell_classref.md#Engine.prototype.preloadFile) method.

Optionally, you can also manually [`engine.init`](html5_shell_classref.md#Engine.prototype.init) to perform specific actions after
...
To load the engine manually the [`Engine.load()`](html5_shell_classref.md#Engine.load) static method must be called. As
...
To prevent this from happening see the [`unloadAfterInit`](html5_shell_classref.md#unloadAfterInit) override option. It is still possible
to unload the engine manually afterwards by calling the [`Engine.unload()`](html5_shell_classref.md#Engine.unload) static method. Unloading the engine
...
you can call the [`Engine.isWebGLAvailable()`](html5_shell_classref.md#Engine.isWebGLAvailable) method. It optionally takes an argument that
...
[OS.get_executable_path()](../../../classes/class_os.md#class-os-method-get-executable-path) method and defines the name of
the automatically started main pack. The [`executable`](html5_shell_classref.md#executable) override option can be
...
element the [`canvas`](html5_shell_classref.md#canvas) override option can be used. It requires a reference to the DOM
element itself.
...
The way the engine resize the canvas can be configured via the [`canvasResizePolicy`](html5_shell_classref.md#canvasResizePolicy)
override option.

If your game takes some time to load, it may be useful to display a custom loading UI which tracks
the progress. This can be achieved with the [`onProgress`](html5_shell_classref.md#onProgress) callback option, which
...
If your game supports multiple languages, the [`locale`](html5_shell_classref.md#locale) override option can be used to
...
Use the [`onPrint`](html5_shell_classref.md#onPrint) override option to set a callback function for the output stream,
and the [`onPrintError`](html5_shell_classref.md#onPrintError) override option to set a callback function for the error stream.
