<a id="doc-internationalizing-games"></a>
...
[Importing translations](../assets_pipeline/importing_translations.md#doc-importing-translations) tutorial. If you haven't read the Importing
...
<a id="doc-internationalizing-games-configuring-imported-translation"></a>
...
![image](tutorials/i18n/img/localization_dialog.webp)
...
![image](tutorials/i18n/img/localization_remaps.webp)
...
obtained via [OS.get_locale_language()](../../classes/class_os.md#class-os-method-get-locale-language).
If your game is not available in that language, it will fall back to the
[Fallback](../../classes/class_projectsettings.md#class-projectsettings-property-internationalization-locale-fallback)
in Project > Project Settings > General > Internationalization > Locale, or to `en` if empty.
...
A [locale](locales.md#doc-locales) is commonly a combination of a language with a
...
Some controls, such as [Button](../../classes/class_button.md#class-button) and [Label](../../classes/class_label.md#class-label),
...
In code, the [Object.tr()](../../classes/class_object.md#class-object-method-tr) function can be used.
...
A good resource for multilingual fonts is [Noto Fonts](https://www.google.com/get/noto/).
...
[GDScript format strings](../scripting/gdscript/gdscript_format_string.md#doc-gdscript-printf) or the equivalent feature in C#. This lets
...
<a id="doc-internationalizing-games-translation-contexts"></a>
...
<a id="doc-internationalizing-games-pluralization"></a>
...
this, make sure to read the tutorial on [Size and anchors](../ui/size_and_anchors.md#doc-size-and-anchors), as
dynamically adjusting control sizes may help.
[Container](../../classes/class_container.md#class-container) can be useful, as well as the text wrapping
options available in [Label](../../classes/class_label.md#class-label).

To check whether your UI can accommodate translations with longer strings than
the original, you can enable [pseudolocalization](pseudolocalization.md#doc-pseudolocalization)
...
called the [TranslationServer](../../classes/class_translationserver.md#class-translationserver).
Translations can be added or removed during runtime;
the current language can also be changed at runtime.

<a id="doc-internationalizing-games-bidi"></a>
...
![image](tutorials/i18n/img/ui_mirror.png)
...
To include it, go to Project > Project Settings > General > Internationalization > Locale
and enable Include Text Server Data, then export the project.
...
![image](tutorials/i18n/img/bidi_override.png)
...
[TextServer.format_number(string, language)](../../classes/class_textserver.md#class-textserver-method-format-number)
can be used to convert Western Arabic numbers (0..9) to the localized numbering system
and [TextServer.parse_number(string, language)](../../classes/class_textserver.md#class-textserver-method-parse-number)
to convert it back.
...
Under Project > Project Settings > General > Internationalization > Locale
(with advanced settings enabled) is a Test property. Set this property
...
![image](tutorials/i18n/img/locale_test.webp)
...
Second, from within the editor go to the top bar and click on View on the top bar,
then go down to Preview Translation and select the language you want to preview.

![image](tutorials/i18n/img/locale_editor_preview.webp)

All text in scenes in the editor should now be displayed using the selected language.

Translations can also be tested when [running Godot from the command line](../editor/command_line_tutorial.md#doc-command-line-tutorial).
...
than one language go to Project > Project Settings > General > Application >
Config. From here click on the Localizable String (Size 0) button,
then the Add Translation button.
...
![image](tutorials/i18n/img/localized_name.webp)

If you are unsure about the language code to use, refer to the
[list of locale codes](locales.md#doc-locales).
