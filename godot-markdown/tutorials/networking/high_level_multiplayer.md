<a id="doc-high-level-multiplayer"></a>
...
Godot always supported standard low-level networking via ,  and some higher-level protocols such as  and .
...
In summary, you can use the low-level networking API for maximum control and implement everything on top of bare network protocols or use the high-level API based on [SceneTree](../../classes/class_scenetree.md#class-scenetree) that does most of the heavy lifting behind the scenes in a generally optimized way.
...
[https://gafferongames.com/post/udp_vs_tcp/](https://gafferongames.com/post/udp_vs_tcp/)
...
Godot uses a mid-level object [MultiplayerPeer](../../classes/class_multiplayerpeer.md#class-multiplayerpeer).
This object is not meant to be created directly, but is designed so that several C++ implementations can provide it.

This object extends from [PacketPeer](../../classes/class_packetpeer.md#class-packetpeer), so it inherits all the useful methods for serializing, sending and receiving data. On top of that, it adds methods to set a peer, transfer mode, etc. It also includes signals that will let you know when peers connect or disconnect.

This class interface can abstract most types of network layers, topologies and libraries. By default, Godot
provides an implementation based on ENet ([ENetMultiplayerPeer](../../classes/class_enetmultiplayerpeer.md#class-enetmultiplayerpeer)),
one based on WebRTC ([WebRTCMultiplayerPeer](../../classes/class_webrtcmultiplayerpeer.md#class-webrtcmultiplayerpeer)), and one based on WebSocket
([WebSocketMultiplayerPeer](../../classes/class_websocketmultiplayerpeer.md#class-websocketmultiplayerpeer)), but this could be used to implement
...
When hosting a server, clients on your  can
...
residential connections use a [NAT](https://en.wikipedia.org/wiki/Network_address_translation). Godot's
...
use [this website](https://icanhazip.com/) to find your public IP address.
...
High-level networking in Godot is managed by the [SceneTree](../../classes/class_scenetree.md#class-scenetree).
...
<a id="doc-high-level-multiplayer-rpcs"></a>
...
See further explanation and troubleshooting on [this post](https://github.com/godotengine/godot/issues/57869#issuecomment-1034215138).
...
- `"authority"`: Only the multiplayer authority can call remotely.
  ...
  [Node.set_multiplayer_authority](../../classes/class_node.md#class-node-method-set-multiplayer-authority).
...
[Exporting for dedicated servers](../export/exporting_for_dedicated_servers.md#doc-exporting-for-dedicated-servers) for more information.
