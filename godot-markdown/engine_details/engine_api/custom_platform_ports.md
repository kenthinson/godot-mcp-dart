<a id="doc-custom-platform-ports"></a>

# Custom platform ports

Similar to [Custom modules in C++](custom_modules_in_cpp.md#doc-custom-modules-in-cpp), Godot's multi-platform architecture
...
it does not use the [DisplayServer](../../classes/class_displayserver.md#class-displayserver) abstraction that is new in Godot 4.

Some reasons to create custom platform ports might be:

- You want to port your game to consoles
  (see also the [Godot website on console support](https://godotengine.org/consoles/)),
...
similar, with the exception of [DisplayServer](../../classes/class_displayserver.md#class-displayserver) not being split
from the [OS](../../classes/class_os.md#class-os) singleton.
...
At the very least, a platform port must have methods from the [OS](../../classes/class_os.md#class-os)
...
- [Export handler](https://github.com/godotengine/godot/tree/master/platform/linuxbsd/export)
  (for exporting from the editor, including [One-click deploy](../../tutorials/export/one-click_deploy.md#doc-one-click-deploy)).
  ...
  [One-click deploy](../../tutorials/export/one-click_deploy.md#doc-one-click-deploy) is implemented for the target platform. This icon
...
- Use a library at runtime to translate Vulkan or OpenGL calls to another graphics API.
  For example, [MoltenVK](https://moltengl.com/moltenvk/) is used on macOS
...
[Custom modules in C++](custom_modules_in_cpp.md#doc-custom-modules-in-cpp), this allows for streamlining the build process
